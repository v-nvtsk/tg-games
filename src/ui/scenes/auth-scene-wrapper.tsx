...
function randomFirefly(color: string) {
  // x, y — стартовая позиция (0-90%), dx, dy — смещение (px)
  return {
...
export const AuthSceneWrapper: React.FC = () => {
  ...
  // Fireflies state
  const [firefliesLeft, setFirefliesLeft] = useState<any[]>([]);
  const [firefliesRight, setFirefliesRight] = useState<any[]>([]);

  useEffect(() => {
    const leftInterval = setInterval(() => {
      setFirefliesLeft(f => [...f, randomFirefly('#3b82f6')]);
    }, 220);
    const rightInterval = setInterval(() => {
      setFirefliesRight(f => [...f, randomFirefly('#fb7185')]);
    }, 220);
    return () => {
  ...
  // Удаляем старых светлячков
  useEffect(() => {
    if (!firefliesLeft.length && !firefliesRight.length) return;
    const timeout = setTimeout(() => {
      setFirefliesLeft(f => f.filter(fly => Date.now() - (fly.createdAt || 0) < fly.lifeTime));
      setFirefliesRight(f => f.filter(fly => Date.now() - (fly.createdAt || 0) < fly.lifeTime));
    }, 800);
    return () => clearTimeout(timeout);
  }, [firefliesLeft, firefliesRight]);

  // Добавляем createdAt при создании
  function withCreatedAt(arr: any[]) {
    return arr.map(f => f.createdAt ? f : { ...f, createdAt: Date.now() });
  }
  useEffect(() => { setFirefliesLeft(withCreatedAt); }, [firefliesLeft.length]);
  useEffect(() => { setFirefliesRight(withCreatedAt); }, [firefliesRight.length]);

  const handleGenderSelect = (gender: "boy" | "girl") => {
    console.log("handleGenderSelect", gender);
    ...
    <div className={styles.sceneWrapper}>
      <div className={styles.gameContainer}>
        <div className={styles.contentWrapper}>
          <div className={styles.gameTitle} onClick={() => console.log("name clicked")}>
            <h1 className={styles.mainTitle}>Ветви и корни</h1>
            <p className={styles.subtitle}>Начни свое приключение</p>
          </div>
        </div>
        {/* Поле ввода имени и кнопка только если выбран пол, теперь внизу экрана */}
        {selectedGender && (
        ...
        <div className={styles.squaresRow} onClick={() => console.log("squaresRow clicked")}>
          <div className={styles.squareGlowWrapper} onClick={() => handleGenderSelect("boy")} tabIndex={0}>
            {selectedGender === "boy" && <div className={styles.fireflies}>
              {firefliesLeft.map(fly => (
                <div
                  key={fly.id}
          ...
          <div className={styles.squareGlowWrapper} onClick={() => handleGenderSelect("girl")}>
            {selectedGender === "girl" && <div className={styles.fireflies}>
              {firefliesRight.map(fly => (
                <div
                  key={fly.id}
                  className={styles.firefly}
                  style={{
